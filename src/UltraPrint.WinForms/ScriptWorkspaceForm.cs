...
public sealed class ScriptWorkspaceForm : Form
{
    ...
    private bool CompileCurrent()
    {
        ...
        try
        {
            ...
            var applicationRoot = GetScriptApplicationRoot();
            var facades = LegacyScriptCoreFacadeRegistration.Register(_session, applicationRoot);
            var interpretationContext = new LegacyScriptInterpretationContext(
                applicationRoot,
                "UltraPrint",
                new WinFormsLegacyScriptInteraction(this),
                facades.Variables);
            var plan = _session.LoadProgram(_code.Text, _sourcePath, interpretationContext);
            if (plan.Cancelled)
            {
                AppendDiagnostic("Compilation cancelled by a legacy Interpretariga interaction macro.");
                DisposeSession();
                return false;
            }
            ...
            AppendDiagnostic($"Compiled with {_session.EngineDescription} using the recovered SostituisciRiga -> Interpretariga -> AddProg pipeline.");
            AppendDiagnostic($"Registered proven facades: Me.Chiudimi; Funzioni.GetVariabile/SetVariabile/IncVariabile/Sostituisci; File.GetIni/WriteIni/SoloExt/SoloNomeFile/SoloPath/Esiste; App.Path/EXEName (App.Path = {applicationRoot}).");
            AppendDiagnostic("Remaining AddObjects form/device facades are still intentionally unavailable until their callable contracts are proven.");
            return true;
        }
    ...
    private IEnumerable<string> GetCandidateApplicationRoots()
    {
        ...
        var legacyRoot = TryGetLayoutApplicationRoot();
        if (!string.IsNullOrWhiteSpace(legacyRoot) && !string.Equals(legacyRoot, current, StringComparison.OrdinalIgnoreCase))
            yield return legacyRoot;
    }

    private string GetScriptApplicationRoot() =>
        TryGetLayoutApplicationRoot() ?? Path.GetFullPath(AppContext.BaseDirectory);

    private string? TryGetLayoutApplicationRoot()
    {
        if (_layout?.SourcePath is not { Length: > 0 } layoutPath) return null;
        var layoutDirectory = Path.GetDirectoryName(Path.GetFullPath(layoutPath));
        if (layoutDirectory is null) return null;
        if (!string.Equals(Path.GetFileName(layoutDirectory), "Ly", StringComparison.OrdinalIgnoreCase)) return null;
        return Directory.GetParent(layoutDirectory)?.FullName;
    }
