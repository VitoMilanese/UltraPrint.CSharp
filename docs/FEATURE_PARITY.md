...
| `Sequenza` | Page imposition, rows/columns, margins, spacing, horizontal/vertical fill, virtual sheet formats, paper orientation, front/back, page navigation, record search/positioning, print page/all | **Partial/strong** | Managed Sequence workspace implements rows/columns, recovered `MargineDestro` left-origin X offset, top margin, native `Passo*` inter-card gaps, `Orizzontale` row-major / `Verticale` column-major fill, `Campo.ini [Formati]` -> `cboDimensioni` virtual sheet sizing, `FoglioPortrait` / `FoglioLandscape`, first-slot selection, sheet navigation, Front/Back/Both, cut marks, Page Setup and current/all sheet preview/print. The print path compensates modern printable-area hard-margin origin to retain legacy physical-page coordinates. Legacy `.Seq` persistence covers the proven controls; `Pescarecord` and `PosizionaPagina` preserve recovered native behavior. Exact `cboDimensioni` -> physical printer paper-size behavior, real-driver clipping validation and device-specific duplex transforms remain. |
...
| --- | --- | --- |
| `Campo.ini` | **Partial/strong** | ANSI/order-preserving reader/writer and `[$]` current-value semantics recovered. Sequenza now reproduces the native `[Formati]` keys 1..20 lookup and `[widthxheight cm]` virtual-sheet parsing. More consumers still need implementation. |
...
| `.sequence.json` | **Managed compatibility state** | Version 4 persists recovered `Passo*` gap semantics, fill direction, paper orientation and raw `cboDimensioni` text. v3 migrates with native first-`[Formati]` fallback; v2 migrates to Portrait; v1 full-pitch files migrate without changing physical placement. Proven legacy controls (`Righe`, `Colonne`, `MargineDestro`, `MargineAlto`, `Passo*`, `Orizzontale`, `Verticale`, `cboDimensioni`, `FoglioPortrait`, `FoglioLandscape`, `PaginaSingola`) are also read/written through `[Sequenza]` `.Seq`. |
