...
| `smartDriver` | printer API version/model/serial/error, magstripe encode/read/print, cleaning, firmware, smart-card init | **Partial/strong** | The 20 recovered smartDriver procedures and exact 24-export ICE_API x86 surface are documented. Managed code isolates the vendor runtime, reproduces read-only printer info/status, and now implements the exact `PrintWithCardStatus` `GetCardId -> GetCardStatus(level 1)` bounded monitor semantics (maximum 60 immediate calls). Full mutating `StampaRecord` module/branch orchestration, any proven device cancellation path, magstripe/chip and maintenance actions remain disabled pending exact recovery/hardware validation. |
