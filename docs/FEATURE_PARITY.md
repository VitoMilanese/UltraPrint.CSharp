...
| `Sequenza` | Page imposition, rows/columns, margins, pitch, front/back, page navigation, print page/all | **Partial/strong** | Managed Sequence workspace implements rows/columns, margins, pitch, first-slot selection, sheet navigation, Front/Back/Both, cut marks, Page Setup and current/all sheet preview/print. Exact legacy setup persistence and printer-specific duplex transforms remain. |
...
| record/batch print | `StampaRecord`, `StampaTutti` | **Partial/strong** | Current record, all loaded records, and sheet-imposed sequences can preview/print using the same card renderer. Device-specific and exact legacy sequence setup behavior remain. |
...
| `.sequence.json` | **Managed compatibility state** | Non-destructive sheet-imposition settings for the managed Sequenza workflow until native `LeggiSetup`/`ScriviSetup` persistence is decoded. |
