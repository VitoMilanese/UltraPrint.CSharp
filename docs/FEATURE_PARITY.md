...
| `Sequenza` | Page imposition, rows/columns, margins, spacing, horizontal/vertical fill, front/back, page navigation, record search/positioning, print page/all | **Partial/strong** | Managed Sequence workspace implements rows/columns, the recovered `MargineDestro` left-origin X offset, top margin, native `Passo*` inter-card gaps, `Orizzontale` row-major / `Verticale` column-major fill, first-slot selection, sheet navigation, Front/Back/Both, cut marks, Page Setup and current/all sheet preview/print. Legacy `.Seq` persistence covers the proven controls; script-visible `Pescarecord` reproduces FormQBE/FindFirst and `PosizionaPagina(NumRecord)` preserves native page arithmetic plus MSFlexGrid highlighting. Paper-orientation/printer-origin behavior and device-specific duplex transforms remain. |
...
| record/batch print | `StampaRecord`, `StampaTutti` | **Partial/strong** | Current record, all loaded records, and sheet-imposed sequences can preview/print using the same card renderer. Printer-origin, duplex and device-specific behavior remain. |
...
| Original object/workflow | Recovered behavior | Status | Remaining work |
| --- | --- | --- | --- |
| standard Windows print path | `MainForm.mnuFilePrint`, preview, page setup, `StampaRecord` | **Partial/strong** | Managed PrintDocument supports layout, current/all database records and sequence sheet imposition; parity testing against real printers and cancellation/progress remain. |
...
| `SmartFormDll.Report` | `PrintPage`, `PrintGrid`, layout/field/record source, tracks | **Partial** | Managed card/record renderer plus sequence sheet imposition covers core PrintPage/PrintGrid-style behavior; tracks remain. |
...
| `.sequence.json` | **Managed compatibility state** | Version 2 stores managed-only/unresolved sequence state with recovered `Passo*` gap semantics and fill direction; v1 full-pitch files migrate without changing physical placement. Proven legacy controls (`Righe`, `Colonne`, `MargineDestro`, `MargineAlto`, `Passo*`, `Orizzontale`, `Verticale`, `PaginaSingola`) are also read/written through `[Sequenza]` `.Seq`. |
