...
| `smartDriver` | printer API version/model/serial/error, magstripe encode/read/print, cleaning, firmware, smart-card init | **Partial/strong** | Managed code now captures the proven `StampaRecord` orchestration envelope without executing it: missing Optional Variant defaults False; interactive mode is enabled; Win32 `StartDocA`/`StartPage` pair with script `StartDoc`/`StartPage`; Optional=True additionally enters pre-print magstripe preparation and `_RotateCardSide(TRUE)`; `_FeedCard(hDC, 0x11)` precedes script `EncodeChip`; later `_SmartCardContinue` sites use raw `1/0/1`; `HasRear` gates the late rear continuation; cleanup disables interactive mode. Semantic names for unresolved flags/raw modes and active hardware mutation remain deliberately unguessed/disabled pending hardware validation. |
