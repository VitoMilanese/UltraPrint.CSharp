...
| `frmPrinting` | Print progress/cancel shell | **Partial** | Standard Windows PrintDocument, record/batch and sequence sheet paths exist; progress/cancel and device-specific pipeline remain. |
| `Sequenza` | Page imposition, rows/columns, margins, spacing, horizontal/vertical fill, paper orientation, front/back, page navigation, record search/positioning, print page/all | **Partial/strong** | Managed Sequence workspace implements rows/columns, recovered `MargineDestro` left-origin X offset, top margin, native `Passo*` inter-card gaps, `Orizzontale` row-major / `Verticale` column-major fill, `FoglioPortrait` / `FoglioLandscape`, first-slot selection, sheet navigation, Front/Back/Both, cut marks, Page Setup and current/all sheet preview/print. The print path compensates modern printable-area hard-margin origin to retain legacy physical-page coordinates. Legacy `.Seq` persistence covers the proven controls; `Pescarecord` and `PosizionaPagina` preserve recovered native behavior. Exact `cboDimensioni` paper-size mapping, real-driver clipping validation and device-specific duplex transforms remain. |
...
| record/batch print | `StampaRecord`, `StampaTutti` | **Partial/strong** | Current record, all loaded records, and sheet-imposed sequences can preview/print using the same card renderer. Legacy sequence page-origin coordinates are restored; exact printer clipping, duplex and device-specific behavior remain. |
...
| Original object/workflow | Recovered behavior | Status | Remaining work |
| --- | --- | --- | --- |
| standard Windows print path | `MainForm.mnuFilePrint`, preview, page setup, `StampaRecord` | **Partial/strong** | Managed PrintDocument supports layout, current/all database records and sequence sheet imposition. Sequence printing now translates from .NET's printable-area Graphics origin back to the legacy physical-page origin using printer hard margins. Real-printer clipping parity, cancellation/progress and device-specific paths remain. |
...
| `SmartFormDll.Report` | `PrintPage`, `PrintGrid`, layout/field/record source, tracks | **Partial** | Managed card/record renderer plus sequence sheet imposition covers core PrintPage/PrintGrid-style behavior; recovered sequence page coordinates now use physical-page origin. Tracks and device-specific output remain. |
...
| `.sequence.json` | **Managed compatibility state** | Version 3 persists recovered `Passo*` gap semantics, fill direction and paper orientation. v2 migrates to its previously implicit Portrait default; v1 full-pitch files migrate without changing physical placement. Proven legacy controls (`Righe`, `Colonne`, `MargineDestro`, `MargineAlto`, `Passo*`, `Orizzontale`, `Verticale`, `FoglioPortrait`, `FoglioLandscape`, `PaginaSingola`) are also read/written through `[Sequenza]` `.Seq`. |
