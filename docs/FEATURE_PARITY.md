...
| Original object/workflow | Recovered behavior | Status | Remaining work |
| --- | --- | --- | --- |
| `Opzioni` | large options/configuration UI | **Missing** | Rebuild incrementally as each backed setting is decoded. |
| `File` | INI/files/log/archive/zip/encrypt/decrypt/path helpers | **Partial** | Legacy INI and asset paths exist; archive/encryption and remaining file utilities are not all ported. |
