...
| `smartDriver` | printer API version/model/serial/error, magstripe encode/read/print, cleaning, firmware, smart-card init | **Partial/strong** | The 20 recovered smartDriver procedures and exact 24-export ICE_API x86 surface are documented. Managed code isolates the vendor runtime, validates exports and now reproduces the recovered read-only polling/model/serial/mag-head/active-job/first-error queries with native levels and two-pass buffer contracts. `StampaRecord`, device cancellation, magstripe/chip and maintenance actions remain disabled pending exact orchestration/hardware validation. |
...
| `ICE_API` / printer wrappers | vendor/device API calls | **Partial/strong** | `LegacyIceApiProbe` isolates the external `ICE_API.DLL`, enforces the recovered 32-bit stdcall ABI and catalogs/validates all 24 exact exports. `LegacyIceApiReader` safely reads polling state, model, serial, magnetic-head type, active job id and the first printer error using recovered levels/record sizes; it never clears errors or changes polling state. Card-status/print orchestration and every hardware-mutating export remain disabled until proven and validated on supported hardware. |
