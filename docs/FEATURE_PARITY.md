...
| `Sequenza` | Page imposition, rows/columns, margins, pitch, front/back, page navigation, record search, print page/all | **Partial/strong** | Managed Sequence workspace implements rows/columns, margins, pitch, first-slot selection, sheet navigation, Front/Back/Both, cut marks, Page Setup and current/all sheet preview/print. Legacy `.Seq` INI setup persistence is decoded for the proven controls, and script-visible `Pescarecord` now reproduces FormQBE/FindFirst one-based record selection. Remaining `.Seq` controls, `PosizionaPagina` numbering/geometry and printer-specific duplex transforms remain. |
...
| record/batch print | `StampaRecord`, `StampaTutti` | **Partial/strong** | Current record, all loaded records, and sheet-imposed sequences can preview/print using the same card renderer. Device-specific and exact legacy sequence geometry remain. |
...
| `.sequence.json` | **Managed compatibility state** | Non-destructive state for managed-only/unresolved sequence settings. Proven legacy setup controls are also read/written through the recovered `[Sequenza]` `.Seq` INI format. |
