...
| `frmPrinting` | Database-batch `Inizia` / `Annulla` shell with interval countdown | **Missing** | Native start/cancel and `[Setup] Intervallo` behavior is decoded, but the countdown/delay path is guarded by an unidentified `frmCarta` boolean at vtable `+0x79C`. The managed database-batch shell remains intentionally unimplemented until that gate is identified. `Sequenza.Pausa` / `cmdStop` are separate controls and are already implemented in the Sequence workspace. |
...
| record/batch print | `StampaRecord`, `StampaTutti` | **Partial/strong** | Current record, all loaded records, and sheet-imposed sequences can preview/print using the same card renderer. Legacy sequence page-origin, cut-and-stack, generic back-slot geometry and Sequence Pause/Stop behavior are restored. The separate database-batch `frmPrinting` `Inizia`/`Annulla`/interval shell, exact printer paper selection/clipping, real duplex validation and device-specific behavior remain. |
