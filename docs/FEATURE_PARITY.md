...
| `Sequenza` | Page imposition, rows/columns, margins, pitch, front/back, page navigation, record search/positioning, print page/all | **Partial/strong** | Managed Sequence workspace implements rows/columns, margins, pitch, first-slot selection, sheet navigation, Front/Back/Both, cut marks, Page Setup and current/all sheet preview/print. Legacy `.Seq` persistence is decoded for the proven controls; script-visible `Pescarecord` reproduces FormQBE/FindFirst, and `PosizionaPagina(NumRecord)` now preserves the recovered page arithmetic plus MSFlexGrid record highlighting. Remaining geometry/setup controls and printer-specific duplex transforms remain. |
